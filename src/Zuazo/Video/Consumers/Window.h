...
public:
	...
	static const std::set<std::shared_ptr<const Screen>>& getScreens();
private:
	Screen(const GLFWmonitor * monitor);

	GLFWmonitor	*					m_monitor;

	static const std::set<std::shared_ptr<const Screen>> 	s_screens;
...
inline bool Screen::isAvalible() const{
	std::lock_guard<std::mutex> lock(s_cbkMutex);

	return m_monitor != nullptr;
}

inline const std::set<std::shared_ptr<const Screen>>& Screen::getScreens(){
	std::lock_guard<std::mutex> lock(s_cbkMutex);
