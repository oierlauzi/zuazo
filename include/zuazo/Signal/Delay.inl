...
template<typename T>
Delay<T>::Delay(std::string name, Duration delay)
...
template<typename T>
Layout::PadProxy<Input<T>>& Delay<T>::getInput() {
	return Layout::makeProxy(m_io->input);
}

template<typename T>
const Layout::PadProxy<Input<T>>& Delay<T>::getInput() const {
	return Layout::makeProxy(m_io->input);
}

template<typename T>
Layout::PadProxy<Output<T>>& Delay<T>::getOutput() {
	return Layout::makeProxy(m_io->output);
}

template<typename T>
const Layout::PadProxy<Output<T>>& Delay<T>::getOutput() const {
	return Layout::makeProxy(m_io->output);
}
...
template<typename T>
void Delay<T>::setDelay(Duration delay) {
	m_delay = delay;
}

template<typename T>
Duration Delay<T>::getDelay() const {
	return m_delay;
}
...
template<typename T>
void Delay<T>::update(TimePoint now) {
